...
def get_normals_lateral(coords, l):
    normals = []
    ...
    for i in range(len(coords)):
        ...
        normal = normal / np.linalg.norm(normal) if settings['normalize_manual'] else normal
...
def draw_normals(coords, l):
    normals = get_normals_lateral(coords, l)
    ...
    if settings['display_normals']:
        ...
        for i in range(len(normals)):
            ...
            glVertex3f(coords_new[i][0], coords_new[i][1] + 2*l, coords_new[i][2])
...
def draw_prism():
    ...
    coords_new, n_top, n_btm = draw_normals(coords, l)
    ...
    for i in range(len(coords_new)):
        ...
        if i != len(coords_new) - 1:
            ...
            glVertex3f(coords_new[i+1][0], coords_new[i+1][1] + 2 * l, coords_new[i+1][2])

            glNormal3f(*n_btm[i+1])
            glVertex3f(coords_new[i+1][0], coords_new[i+1][1], coords_new[i+1][2])
        else:
            glNormal3f(*n_top[0])
            glVertex3f(coords_new[0][0], coords_new[0][1] + 2 * l, coords_new[0][2])
...
def draw_light_src(l):
    l0 = [2*l, -l, 0]
    l1 = [0, 3*l, 2*l]
    mtClr0 = [1, 0.4, 0.2] # Цвет материала
    lghtClr0 = [1, 1, 0] # Цвет источника света 0
    lghtClr1 = [1, 0, 1] # Цвет источника света 1
    ...
    glEnd()
    glEnable(GL_LIGHTING) if settings['lightning_active'] else glDisable(GL_LIGHTING)
    ...
    glLightfv(GL_LIGHT0, GL_POSITION, (GLfloat * 4)(*l0, 0))
    ...
    glEnable(GL_LIGHT0)
    glEnable(GL_LIGHT1) if settings['static_source_active'] else glDisable(GL_LIGHT1)
...
def on_draw():
    ...
    glEnable(GL_DEPTH_TEST) if settings['depth_test'] else glDisable(GL_DEPTH_TEST)
    glEnable(GL_CULL_FACE) if settings['cull_face'] else glDisable(GL_CULL_FACE)
    glEnable(GL_NORMALIZE) if settings['normalize_auto'] else glDisable(GL_NORMALIZE)
    show_axis()
    draw_light_src(3)
    draw_prism()
